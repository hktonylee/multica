...
### Traefik (Docker Compose profile)

The self-host compose file includes an optional Traefik service and router labels.

```bash
# Start stack with Traefik enabled
docker compose -f docker-compose.selfhost.yml --profile traefik up -d --build
```
...
You can override hostnames/ports in `.env`:

```bash
TRAEFIK_WEB_HOST=app.example.com
